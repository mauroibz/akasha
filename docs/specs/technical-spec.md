...
**A connector may declare a search phase** (Sprint 083): `search_job = "search_import_rows"`, the same opt-in shape as `browsable` and `incremental`. A connector declaring it is the one whose source carries no identity to trust (an empty `identity_kinds`, DEC-082); preview stages its batches in the `matching` state with exactly one `search_import_rows` job enqueued (owner = the previewing user, payload = `{batch_id}`), and `commit` refuses with 409 until the job has flipped the batch back to `previewed` — written before the job completes so a poller never sees `matching` with a succeeded job.

**The search job walks rows sequentially** — one row at a time, no concurrency across rows, the owner's explicit instruction — querying `"<title> <author>"` (both, because a Spanish title often needs its author to disambiguate the translation) through **the row's own domain's** enabled providers (resolved from the record's `item_type`, the connector's declared types as the fallback — the interactive search's provider-scoping rule applied per row, so a provider of another domain is never asked and never spends its budget on a row it cannot serve), paced by the runner's `RateLimiter` and budgeted by `ProviderQuota` (DEC-045's background half: a capped provider is skipped per row, and when no provider remains the job **defers without spending an attempt** to the next day's window). Each row's merged top three results are stored as **proposals** — `import_proposals` (migration 0022): one row per provider result, `rank` is `merge_and_rank`'s position (never a confidence the connector would be inventing), `chosen` is the owner's answer (NULL / confirmed / discarded), and the `payload_json` holds everything the confirmation re-stages from. Failure containment: a row whose providers all fail is `search_failed` in the job's progress and stays discardable — the job never dead-letters for per-row provider errors. A provider returning zero results is "no results", an answer, not an error. Undo cancels an in-flight search job the way it cancels enrichment.

**Confirming a proposal re-stages the record's item half** from the proposal's payload (title, creators, year, identifiers, metadata, language) and recomputes `planned_action` against the library through the same `ImportMatcher` preview used — so commit needs no new path, and an exact identity may now resolve `reuse_item` where the typed row could not. The confirmed identity reaches the item through a `confirmed_identifiers` key on the stored payload, **normalized through the same identity rule the add path applies** (a provider's `isbn13` lands as the canonical `isbn` — the enrichment join and the library's exact-identity match key on the canonical kind; the post-commit enrichment backfill installs the confirmed row's cover): `identity_kinds` governs what the *reader* may trust, not what an explicit owner confirmation stored, and the key is present only on confirmed rows so every other connector's commit is unchanged. Confirming also stashes the typed item half so **discarding after a confirm restores the row exactly as the spreadsheet wrote it** (re-planned through the connector's own match); a discard on a never-confirmed row just marks its proposals `chosen=false`. Both answers require a drained batch; both are user-scoped like every other import row. The route is `POST /{importer_name}/batches/{batch_id}/records/{record_id}/proposal` with `{source, source_id}` or `{discard}`; the lightweight `GET /{importer_name}/batches/{batch_id}` re-reads one previewed batch (the poll), and while a batch is `matching` its response carries the job's live `search_progress`.

**A row can be excluded from the commit entirely** (the owner's 2026-09-14 decision): `POST .../records/{record_id}/exclude` sets the row's `planned_action` to `excluded` — the same skip set commit already applies to reader-error rows, so commit needs no new path — and `POST .../records/{record_id}/include` undoes it, re-planning through the connector's own match. Every owner answer that changes a row's fate rewrites the batch's stored summary from the live rows, so the commit gate's counts stay honest.

**Any row can be edited and re-searched** (the owner's 2026-09-14 decision, available on every row — a bad query can also produce wrong proposals): `POST .../records/{record_id}/search` with `{title?, author?}` re-stages the row's item half with the edited text (the spreadsheet's own cells stay in `source_fields`), replaces the record's proposals with fresh results, and clears any earlier answer — a re-search asks a different question. The spend is interactive-shaped (DEC-045): recorded, never blocked. The per-row search seam (`search_row`) is the one the background job and this route share, so the row's domain-scoping rule lives in exactly one place. **Ten proposals are stored per row** (`TOP_N = 10`); the screen renders three and unfolds the rest behind a "Show more".

**The separator pick and the sample row** (the owner's 2026-09-15 refinement): `delimiter` is a declared field on the list connector (the same `fields` channel the column numbers ride; the route forwards exactly what was declared). The reader takes the declared separator over sniffing, refuses an unknown one by name (comma, semicolon or tab — nothing else), and composes the fingerprint with it, so the same bytes read with two separators are two imports. The screen's sample row is display-only: the first data row split on the chosen (or sniffed) separator with the current mapping applied, so the owner can see the configuration working before preview; the preview remains the truth.

**The list takes typed rows and an optional creator column** (Sprint 085, DEC-162): the screen's editor synthesizes the upload the connector declared (`pasted-list.csv`, `text/csv`) — same route, same reader, no new API surface — and a dropped text file fills the editor (a `FileReader`, so jsdom and the browser agree), with editing the editor making it the source. `ImportInputSpec.flags` declares a connector's boolean options the way `fields` declares its string ones (the route forwards exactly what was declared, never a name it did not); the list declares `no_creators`, and the reader honors it by mapping titles only on any domain — the same rule a no-creator-words domain follows — lifting the two-column minimum, refusing an `author_column` sent beside it as the contradiction it is, and never firing the positional creator fallback. `previewImportWithOptions` carries the library pick, so a mapping or flag no longer silently drops the chosen domain.

**The list connector is domain-declared** (Sprint 084, DEC-160): the reader lives in `domain/list.py` (no domain's package) and serves exactly the domains that declare a `Domain.list_columns: ListColumnSpec` — title header words, optional creator header words, the creator column's label. `None` is the deliberate default: a domain that has not declared its list vocabulary is not list-importable, rather than silently borrowing another domain's words. Books keep their Sprint 083 word lists verbatim; album adds artist words; movie, series and anime declare title words only, and for them a missing creator column is an empty fact, not an error. The target library reaches the reader through the same options channel the column mapping rides (`ImportSource.options["targets"]`, forwarded by the upload route from the `targets` form field): the choice is required, exactly one, and **composes the fingerprint** (`#book`, `#album` — one list, one library per batch), with the service trusting a reader that consumed the choice (`source_descriptor.item_type`) instead of double-composing its subset suffix. `ImportInputSpec.single_domain_pick` declares that the connector's target is this single pre-source choice, so the screen renders one dropdown (not the tick-many checkboxes) and labels the mapping fields in the picked domain's own words (its `creators` field label).

**A confirmed card's cover rides the commit** (Sprint 084's walkthrough finding): confirming a proposal fetches its `cover_url` through the app's provider client — allowlist, size and pixel bounds, `prepare_cover` — and stages it on the row's payload through the same `cover_stage` channel every other connector uses, so commit's existing install loop installs it. Never fatal: a failed fetch stages nothing and refuses no confirmation. Discard drops the staged cover with the rest of the provider data. This channel — not the enrichment backfill — is the import's cover path: the backfill is identifier-keyed by design (books by ISBN, albums by Spotify id, DEC-052), and a MusicBrainz release-group offers no identifier at all, so enrichment can never be the only way a confirmed row gets its cover.
...
- Bulk mutation accepts either explicit `entry_ids` or a validated server-side filter plus `excluded_entry_ids`; never both. This supports select-all across unloaded virtual rows without sending thousands of IDs. Return affected count and apply in one transaction. `DELETE /entries/bulk` accepts the same selection shapes with no `set` and removes the selected entries in one transaction — the triage Discard action; a missing id in an explicit selection is the same 404 as every other bulk endpoint, and shelf/format rows cascade with the entry (migration 0015).
...
- The selection's action bar carries the destructive **Discard** as a red button, never a per-row control: the checkbox selection is the pointer boundary and the confirmation dialog is the second deliberate click, which is the click-error safety the row flow could not offer. It sends `DELETE /entries/bulk` with the same selection shape as every other bar action and invalidates Triage and Library queries on success. A detail page opened from a triage row navigates with `state.from = "triage"` and its back control reads "← Triage" returning to `/import?tab=triage`; a deep link with no state keeps "← Library", the default every other caller of the shared back control uses.
