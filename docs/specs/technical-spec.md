...
- In Triage, pointer selection is checkbox-only and row-body clicks open detail. The native score select patches one entry immediately through `PATCH /entries/{id}` with an optimistic snapshot/rollback. A row status select instead updates a client draft map; changing it back to the persisted status removes the draft. Apply groups draft IDs by status and sends one `PATCH /entries/bulk` per distinct value, then invalidates Triage and Library once. Successful groups clear; failed groups remain for retry and produce one announcement. Explicit checkbox bulk actions still commit immediately. Native selects remain deliberate in this fixed-height virtual row: an expanding score picker is clipped by the row geometry, while a portalled select hides the route containing its focused trigger and fails the `aria-hidden-focus` gate (DEC-086, DEC-087).
