...
- In Triage, pointer selection is checkbox-only and row-body clicks open detail. The native score select patches one entry immediately through `PATCH /entries/{id}` with an optimistic snapshot/rollback. A row status select represents a **target**, not the persisted `unsorted` value: explicit draft → imported `suggested_status` → the row domain's `default_status`. It offers only that domain's `choosable` statuses, so Inbox is neither displayed nor selectable. Changing the target updates a client draft map; changing it back to the suggestion/default removes the explicit draft. The row Apply action commits its displayed target through the same grouped bulk mutation, while the sticky Apply action groups every explicit draft by status. Both send one `PATCH /entries/bulk` per distinct value and invalidate Triage and Library once. Successful attempted drafts clear without disturbing unattempted rows; failed ones remain for retry and produce one announcement. Explicit checkbox bulk actions still commit immediately. Native selects remain deliberate in this fixed-height virtual row: an expanding score picker is clipped by the row geometry, while a portalled select hides the route containing its focused trigger and fails the `aria-hidden-focus` gate (DEC-086, DEC-087, DEC-095).
