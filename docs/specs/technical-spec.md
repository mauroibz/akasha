...
- **The library virtualizes against the window, not against a scroll container of its own.** The primary surface uses the whole page, so the list element has no height and no overflow, and the virtualizer is given a `scroll margin` for what sits above it. The mounted-DOM bounds are unchanged by this and are re-asserted at 10,000 entries against the window; triage keeps its own scroll container, sized to its rows for a short inbox and capped at `min(70vh, 760px)` for a long one.
...
- In triage, pointer selection is checkbox-only. Row-body clicks open detail. The row's native status and score selects patch one entry through `PATCH /entries/{id}` with an optimistic snapshot/rollback; bulk and keyboard actions continue to use `PATCH /entries/bulk`. Native selects are deliberate in this fixed-height virtual row: an expanding score picker is clipped by the scroll edges, while a portalled select hides the route containing its focused trigger and fails the `aria-hidden-focus` gate (DEC-086).
