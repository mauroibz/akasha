...
- **Dense table by default** — small cover thumb, title, author, one target status,
  and directly editable score. The target is the importer's suggestion when present and
  otherwise the domain's declared default; Inbox is implied by this screen and is not an
  option. Compact rows, ~25 visible at once;
