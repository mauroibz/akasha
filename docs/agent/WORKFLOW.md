...
After the final planned sprint passes — Sprint 027 under roadmap revision 9 — set project state to `complete`, set `active_sprint` and `active_sprint_file` to `null`, preserve the completed list, and write a release-state handoff. Do not tag, publish, deploy, or push unless the user asks.
