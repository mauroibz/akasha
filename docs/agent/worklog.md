...
## 2026-09-01 — Sprint 060 (complete) — final planned sprint; project is now complete

- Done, all five deliverables: (1) `reclaim_import_batches` collects a
  committed batch's staging directory once its undo window passes, wired
  into `JobRunner`'s idle tick, no `--apply` gate (undo never reads that
  directory — grepped every reference to confirm). (2) `_share_covers`
  hardlinks covers from the live store like DEC-047 already does for
  attachments, deliberately with no sibling-backup fallback (a cover is
  not content-addressed, so a sibling's copy is not guaranteed current);
  `/data/imports` is no longer archived at all; `MANIFEST_VERSION` 1 -> 2,
  both versions still restore, proved against a real pre-Sprint-060
  fixture backup. (3) `akasha-backup prune-pre-migration` lists and,
  named plus `--apply`, deletes pre-migration backups, refusing the
  newest and the current-revision one even when named. (4)
  `ensure_free_space` guards attachment upload, import preview, file
  attach, cover replace and backup creation with a typed 507;
  `/api/health/ready` reports disk state but stays 200 (a full disk can
  still read). (5) the upload branch now reads a connector's declared
  `max_bytes`/`max_files` instead of the shared default. DEC-123 records
  all five decisions with reasoning.
- Verified: full gate. `make check`, `make test` (1,215 backend + 194
  frontend, both green), `make smoke-container` all green.
  AC3 measured directly: two backups of an unchanged 11-cover library
  cost 350,101 bytes (disk-distinct, by inode) against 700,202 for two
  full copies. AC4 proved against `tests/fixtures/backup-v1/`, a real
  backup generated by the actual pre-Sprint-060 code before it changed,
  not a hand-edited manifest. A real narrated backup/restore walkthrough
  ran against the built container with covers uploaded through the live
  API: incidentally exercised the EXDEV (no-hardlink) path because this
  environment's bind-mount setup returns `Invalid cross-device link`
  despite matching `st_dev` — confirmed with a raw `os.link` probe, a
  property of the environment rather than a code defect — and the
  no-sibling-fallback design proved itself there too: the unchanged cover
  got a new inode each backup but stayed byte-identical, the changed
  cover differed correctly in each, verify and restore both succeeded.
- Deviations: `make smoke-container` was not extended with its own
  version-1 restore drill (the sprint's Verification section suggested
  it) — the unit-level fixture test proves the same code path more
  precisely; recorded rather than silently skipped. Inline comments and
  one already-pushed commit message say "DEC-124" (written before
  DEC-122 was confirmed as the prior number) — corrected to DEC-123
  everywhere except that one pushed commit message, per the rule against
  rewriting pushed history.
- Blocked/open: nothing.
- **This was the final planned sprint** (`FINAL_SPRINT = 60` in
  `scripts/validate_project.py`). `docs/agent/state.json` now reads
  `project_status: "complete"` with `active_sprint`/`active_sprint_file`/
  `active_sprint_status` all `null`, per `WORKFLOW.md`'s final-sprint
  rule. No tag was cut for v1.5.6 and nothing beyond the already-
  established push pattern was pushed — the final-sprint rule says not
  to tag/publish/deploy beyond what the user asks, and unlike Sprint
  058/059 nothing in this sprint's acceptance criteria needed a live
  release to prove.
...
## 2026-09-03 — Enrichment now records the resolved provider source (DEC-130, complete)
- Done: the owner checked the new `Fetch cover` button against their own Docker
  instance and found "This item has no provider source" on a Letterboxd-imported,
  fully-enriched movie ("Obsession"). Traced it to a real, pre-existing gap
  affecting every import connector and every domain: `application/add.py`'s
  search-add path writes an `ItemSourceRow` (`is_primary=1`); no import connector
  ever does, and `EnrichmentHandler.process` (`application/enrichment.py`) filled
  metadata/cover from a resolved provider without ever recording that provider as
  the item's source. `primary_source()` therefore always failed for an imported
  item, however completely it had been enriched — silently affecting Sprint 064's
  own Spotify-album path too, not only Letterboxd. Fixed by writing
  `payload_data.source_refs` as `ItemSourceRow`s (primary on the one matching
  `payload_data.source`) inside enrichment's existing write transaction, guarded
  on the item having no source row at all so a search-added or already-enriched
  item is never touched twice.
  Also extended `_backfillable_items` (the query behind the existing operator
  endpoint `POST /api/enrichment/backfill`) with an unconditional "no source at
  all" clause, so that one existing call retroactively re-queues every
  already-enriched, now-stuck item across every domain — no new endpoint, no
  database surgery, and no per-item action needed for the owner's real data.
- Verified: `test_movie_enrichment_fills_only_what_is_empty` and
  `test_an_album_is_enriched_from_its_spotify_identity` both gained a
  source-row assertion against real recorded fixtures (Wikidata, MusicBrainz);
  a new `test_an_item_that_already_has_a_source_is_not_given_a_second_one` proves
  the guard; a new `test_backfill_reaches_a_complete_item_that_was_never_given_a_
  source` proves the retroactive backfill path directly against an
  Obsession-shaped fixture (full metadata/cover/year, no source). Two existing
  backfill tests needed a source added to their "complete, so never queued"
  fixtures, which had themselves never carried one and would otherwise have
  started quietly asserting the old behavior. Full backend suite 1,291 passing.
  `make check` green. No route, schema, or contract changed — verified by
  diff — so neither OpenAPI nor e2e needed rerunning for this piece.
- Deviations: none beyond the fix itself; matches the technical spec's own
  existing wording ("manual-only items have none" — implying every other item
  has one), which the implementation had simply never satisfied.
- Blocked/open: nothing. Same branch (`ui-search-refresh-mini-sprint`), still not
  merged or pushed.
- Next: tell the owner that a single `curl -X POST .../api/enrichment/backfill`
  against their running container will re-queue Obsession (and anything else
...
## 2026-09-13 — Plan revision 41: research and planning for the hand-written-CSV importer (Sprint 083, ready)

- Done: researched the owner's request to import `exports/Libros.csv` (a hand-written CSV, 104
  rows, title/author, no identifiers — unlike every platform-export connector so far) and planned
  it as Sprint 083 rather than building it. Read the whole import boundary live
  (`domain/importers.py`, `application/imports.py`, `api/imports.py`,
  `infrastructure/repositories.py`, the book domain's reader/adapters, the job/quota/enrichment
  infrastructure, `ImportPage`/`TriagePage` and `api/imports.ts`), profiled the real CSV
  (headers with a trailing space, one transposed author/title row, collection volumes, typos,
  quoted titles, dictionaries, 81/22/1 es/en/fr), and put the three material product questions to
  the owner via clarify. Wrote the full plan-revision change set: the Sprint 083 file (created
  `ready`), ROADMAP revision 41 (header, index row, the new "A list you wrote yourself"
  contract section), DEC-156 (the decision, the owner's three answers, the measured baseline),
  `FINAL_SPRINT` 82 → 83 in `scripts/validate_project.py` with the comment-history entry,
  `docs/agent/state.json` regenerated via `sync_sprint_state.py --plan-revision 41`
  (complete → ready, active sprint 083 — DEC-148's no-hand-edit rule held), and this HANDOFF.
- Verified and how: `python scripts/validate_project.py` green after the change set (it first
  caught a missing trailing newline in the new sprint file — fixed); `git diff --check` clean;
  `make test` not owed — no application code changed (docs-only diff, TESTING.md's matrix);
  state.json regenerated by the script, never hand-edited. The owner's three decisions are
  recorded verbatim in the sprint file and DEC-156: (1) provider matching runs as a background
  job with commit blocked until the batch drains, (2) confirmation fills from the provider's
  full payload, (3) only title/author are read — the CSV's other columns are deliberately
  unmapped ("queda a futuro").
- Deviations: none. The original ask was "don't build the importer, do the research to plan the
  sprint" — this session is exactly that, plus the plan-revision commit the protocol requires.
- Blocked/open: none.
- Next: claim Sprint 083 with `sync_sprint_state.py --sprint 083 in_progress` and execute the
  sprint file. Start with D1 (the reader and its column mapping) under TDD; capture the Open
