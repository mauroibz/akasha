...
Observed on 2026-08-27 at `0f1b86e`, the close of the anime line. **Sprints 042 and 043 were
inserted ahead of this one by owner direction and touch only Triage frontend behavior**, so every
backend fact below still holds — but re-check rather than trusting this line.
...
1. `feat(sprint-044): one allowlisting validator for a domain's entry values`
2. `feat(sprint-044): hold a domain to the wiring this build actually has`
3. `test(sprint-044): refuse a CHECK that freezes an application vocabulary`
4. `refactor(sprint-044): construct an entry row in one place`
5. `docs(sprint-044): the rebuild recipe, the pragma, and how to drive these screens`
6. `docs(sprint-044): close sprint and hand off`
