...
| [`export-proposal.md`](export-proposal.md) | Written 2026-09-04 at the owner's request: why the shipped export has no door and only one domain can leave in a format another application reads, what an `ExportView` contract replaces it with, and what that costs. **Accepted as DEC-135 and scheduled as Sprints 068, 069 and 070.** Sprint 070's own scope (an `ExportView` per external ecosystem — MyAnimeList, Letterboxd, an IMDb-shaped file) was built, found to exceed what was actually asked for, and withdrawn the same day; the pre-existing lossless JSON plus Sprint 068's generic `table` CSV are the actual scope and both shipped. **See DEC-136.** | DEC-135 for the decision; Sprints 068–069 for what is built from it; DEC-136 for the withdrawal |
| [`ui-cohesion-proposal.md`](ui-cohesion-proposal.md) | Written 2026-09-04 at the owner's request: the seven rules Sprints 066 and 067 established, eleven places the rest of the application disagrees with them, and what one surface would cost. **Accepted as DEC-136 and scheduled as Sprints 070 and 071** (renumbered from 071–072 into the slot Sprint 070's withdrawal above freed). | DEC-136 for the decision; Sprints 070–071 for what is built from it |
| [`readability-proposal.md`](readability-proposal.md) | Written 2026-09-05 at the owner's request after Sprint 071 closed the last planned v1 sprint, and revised the same day on the owner's reading of it: nineteen measured findings about wasted space and small covers on the library, insights and shelves screens — including that a shelf may hold any domain and no screen says which, so a mixed shelf's count would disagree with the screen it links to — the redraw that answers them, a costed menu of new shelf functionality, and the alternatives rejected for each screen. **Accepted as DEC-139 and scheduled as Sprints 072–074**; its optional saved-views sprint is accepted in principle and left unscheduled. | DEC-139 for the decision; Sprints 072–074 for what is built from it |
| [`auth-and-multiuser-proposal.md`](auth-and-multiuser-proposal.md) | Written 2026-09-07 at the owner's request after v1.8.0 shipped: what `user_id` on `entries` and `shelves` already buys (both unique constraints, six user-leading indexes, a `LibraryService` that already filters on it), what is genuinely missing (a users table, sessions, a user on the request — the service is built 24 times across 8 files with no user argument), and four costed alternatives tables for the mode switch, password hashing, sessions and login friction. **Accepted whole as DEC-146 and scheduled as Sprints 075–082 ending in 2.0.0**; its four open questions are adopted as DEC-146's overridable defaults. | DEC-146 for the decision; Sprints 075–082 for what is built from it |
