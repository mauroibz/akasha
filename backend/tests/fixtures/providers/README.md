...
| `musicbrainz_release_plastic_beach.json` | `GET /release/574166b1-…?inc=artist-credits+labels+media+release-groups+recordings` — measured, not assumed: `_preferred_release`'s tie-break among the group's 18 releases sharing `first-release-date` picks this **same** release the Spotify relation named (lexicographically the smallest id), a coincidence worth recording since a test written from the first few releases seen would guess wrong. This one capture answers both the pass-1 follow-up (`inc=release-groups` is a subset of what it carries) and `fetch()`'s own full read. |
