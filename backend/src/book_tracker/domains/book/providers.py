"""Books' adapters: Open Library and Google Books.

Everything here is ISBN-shaped by nature — the identity that lets two providers merge,
the edition verification DEC-044 added, and the two records themselves. It sits in this
domain's package rather than in `infrastructure/`, so a second domain's adapter is a
file beside its own domain and not another branch in a shared one.
"""
...
class OpenLibraryProvider:
    ...
    def _candidate(self, row: Mapping[str, Any]) -> SearchCandidate | None:
        ...
        return SearchCandidate(
            ...
            year=parse_year(edition.get("publish_date")),
            cover_url=(
            ...
            metadata=metadata,
            original_year=parse_year(row.get("first_publish_year")),
        )
    ...
    async def _edition_payload(
        ...
        original_year = parse_year(work.get("first_publish_date") or created_value)
        metadata = {
        ...
        return ItemPayload(
            ...
            subtitle=str(row["subtitle"]) if row.get("subtitle") else None,
            creators=authors,
            year=parse_year(row.get("publish_date")),
