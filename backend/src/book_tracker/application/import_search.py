...
One job per list-connector preview walks the batch's rows in `row_number` order
and searches the book domain's providers for each — **one row at a time, no
concurrency across rows**, respecting public-API rate limits (the owner's
...
class ImportSearchHandler:
    ...
    async def process(self, job_id: str, now: datetime) -> dict[str, Any]:
        ...
        with Session(self.engine) as session:
            ...
            if batch is None or batch.user_id != user_id or batch.kind != "list":
                return {
                    "state": "failed",
        ...
        for row in rows:
            ...
            try:
                candidates = await search_providers(query, allowed, domain=DOMAINS["book"])
